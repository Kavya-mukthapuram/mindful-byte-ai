...
function getCoachResponse(input: string, userName: string, score: number): string {
  ...
  let base = '';
  if (lower.includes('night') || lower.includes('late') || hour > 21) base = COACH_RESPONSES.night;
  else if (lower.includes('stress') || lower.includes('anxious') || lower.includes('anxiety')) base = COACH_RESPONSES.stress;
  else if (lower.includes('craving') || lower.includes('urge')) base = COACH_RESPONSES.craving;
  else if (lower.includes('sugar') || lower.includes('sweet') || lower.includes('chocolate')) base = COACH_RESPONSES.sugar;
  else if (lower.includes('help') || lower.includes('what can you')) base = COACH_RESPONSES.help;
  else if (lower.includes('binge') || lower.includes('overeating') || lower.includes('overate')) base = COACH_RESPONSES.binge;
  else if (lower.includes('weight') || lower.includes('fat') || lower.includes('lose')) base = COACH_RESPONSES.weight;
  else if (lower.includes('monday') || lower.includes('week')) base = COACH_RESPONSES.monday;
  else if (lower.includes('breakfast') || lower.includes('morning')) base = COACH_RESPONSES.breakfast;
  else base = COACH_RESPONSES.default;
          ...
          <div className="px-4 pb-2 flex gap-2 overflow-x-auto scrollbar-none flex-shrink-0">
            {["Why do I eat at night? 🌙", "Sugar craving help 🍬", "I ate emotionally 😔", "Healthy alternatives 🥗"].map(q => (
              <button key={q} onClick={() => { setInput(q); }}
                className="flex-shrink-0 px-3 py-1.5 bg-primary-light text-primary rounded-xl text-xs font-medium hover:bg-primary hover:text-primary-foreground transition-all">
                {q}
              </button>
            ))}
          </div>
